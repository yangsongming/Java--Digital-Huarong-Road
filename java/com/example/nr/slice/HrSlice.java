...
import com.example.nr.support.Block;
import com.example.nr.support.MyImage;
...
public class HrSlice extends AbilitySlice {
    ...
    public static int jigsawId = ResourceTable.Media_pic7;
    private TableLayout jigsawLayout = null;
    public static PixelMap jigsawPixelMap = null;
...
//    private Map<Integer, Float> blockXMap, blockYMap;   // 用于记录block的x和y
    ...
    private Button btn_pbm;
    public static int xzid;

    @Override
    public void onStart(Intent intent) {
        super.onStart(intent);
        super.setUIContent(ResourceTable.Layout_hr_slice);


        this.jigsawId = intent.getIntParam("jigsawId", ResourceTable.Media_pic7);
    ...
    private void initLayout() {
        ...
        this.stepCntText = (Text) findComponentById(ResourceTable.Id_stepCntText);

        // 添加计时器
        this.tickTimer = (TickTimer) findComponentById(ResourceTable.Id_ticktimer);
        tickTimer.setFormat("mm:ss:SS");//SSSS
        tickTimer.setCountDown(false);

        stepCnt = 0;
        setStepCntText();

        this.jigsawLayout = (TableLayout) findComponentById(ResourceTable.Id_jigsaw_layout);
        ...
        this.btn_begin = (Button) findComponentById(ResourceTable.Id_btn_begin);
        //this.btn_end = (Button) findComponentById(ResourceTable.Id_btn_end);
        this.btn_pbm = (Button) findComponentById(ResourceTable.Id_btn_pbm);

        //btn_end.setClickable(false);

        this.img_tip = (Image) findComponentById(ResourceTable.Id_img_tip);
        ...
        this.backbtn = (Button) findComponentById(ResourceTable.Id_btn_giveup);
    ...
    private void addListener() {
        ...
        btn_pbm.setClickedListener(new Component.ClickedListener() {
            @Override
            public void onClick(Component component) {
                prob();
            }
    ...
    private int getStarCnt() {
        ...
        if (timeperchip <= 0.1) {
            starfortime = 3;      //小于6秒每个
        } else if (timeperchip > 0.1 && timeperchip <= 0.15) {
            starfortime = 2;     //小于9秒每个
        } else {
            starfortime = 1;
        }

        if (stepperchip <= 3) {      //小于三步
            starforstep = 3;
        } else if (stepperchip > 3 && stepperchip <= 5) {    //小于5步
            starforstep = 2;
        } else {
            starforstep = 1;
        }
    ...
    private boolean forgo(){
        ...
        ok.setClickedListener(new Component.ClickedListener() {
            ...
            public void onClick(Component component) {
                cd.destroy();
                AbilitySlice slice = new PrechooseSlice();
    ...
    private boolean prob() {
            CommonDialog cd = new CommonDialog(getContext());
            cd.setCornerRadius(50);
            DirectionalLayout dl = (DirectionalLayout) LayoutScatter.getInstance(getContext()).parse(ResourceTable.Layout_problemdeclare, null, false);

            Button diff_ok1 = (Button) dl.findComponentById(ResourceTable.Id_proreset);
            diff_ok1.setClickedListener(new Component.ClickedListener() {
                @Override
                public void onClick(Component component) {
                    rePlay();
                    cd.destroy();
            ...
            Button proback = (Button) dl.findComponentById(ResourceTable.Id_proback);
            proback.setClickedListener(new Component.ClickedListener() {
            @Override
            public void onClick(Component component) {
                    cd.destroy();
            }
    ...
    private boolean isBINGO() {
        if (getBINGOChipCnt() == jigsawCnt) {
            ...
            Text steptext = (Text) dl.findComponentById(ResourceTable.Id_dialog_step_text);
